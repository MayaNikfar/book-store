...
export async function signUp(userData) {
  ...
  localStorage.setBook('token', token);
  return getUser();
}
...
export function logOut() {
  localStorage.removeBook('token');
}

export function getToken() {
  // getBook will return null if the key does not exists
  const token = localStorage.getBook('token');
